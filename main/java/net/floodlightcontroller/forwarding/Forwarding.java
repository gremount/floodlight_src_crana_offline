...
import net.floodlightcontroller.crana.CRoutingService;
...
public class Forwarding extends ForwardingBase implements IFloodlightModule, IOFSwitchListener {
	...
	protected void preProcess(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx, boolean requestFlowRemovedNotifn) {
		Ethernet eth = IFloodlightProviderService.bcStore.get(cntx, IFloodlightProviderService.CONTEXT_PI_PAYLOAD);
		...
		if (eth.getEtherType() == EthType.IPv4) { /* shallow check for equality is okay for EthType */
			...
			if (FLOWMOD_DEFAULT_MATCH_TRANSPORT) {
				...
				if (ip.getNextHeader().equals(IpProtocol.TCP)) {
					...
					.setExact(MatchField.TCP_SRC, tcp.getSourcePort())
					.setExact(MatchField.TCP_DST, tcp.getDestinationPort());
					System.out.println(id  + "IPv6 tsp = " + tcp.getSourcePort() + "  "  + " tdp = " + tcp.getDestinationPort());
				} else if (ip.getNextHeader().equals(IpProtocol.UDP)) {
					UDP udp = (UDP) ip.getPayload();
					mb.setExact(MatchField.IP_PROTO, IpProtocol.UDP)
					.setExact(MatchField.UDP_SRC, udp.getSourcePort())
					.setExact(MatchField.UDP_DST, udp.getDestinationPort());
					System.out.println(id  + "IPv6 usp = " + udp.getSourcePort() + "  "  + " udp = " + udp.getDestinationPort());
				}
			}
		}
		id++;
